...
class Interfaz(Frame):
    ...
    def creaMenu(self):
        ...
        self.practica3 = Menu(self.menuBar, tearoff=0)
        self.practica3.add_command(label="Colores SIN letras", command=lambda: self.aplicaLetras(1))
        self.practica3.add_command(label="Tonos de gris", command=lambda: self.aplicaLetras(2))
        self.practica3.add_command(label="Letras blanco y negro", command=lambda: self.aplicaLetras(3))
        self.practica3.add_command(label="Colores + letras tonos de gris", command=lambda: self.aplicaLetras(4))
        self.practica3.add_command(label="Tonos de gris + letras tonos de gris", command=lambda: self.aplicaLetras(5))
        self.practica3.add_command(label="Colores con texto", command=self.aplicaPalabras)
        self.practica3.add_command(label="Domino blanco", command=lambda: self.aplicaLetras(6))
        self.practica3.add_command(label="Domino negro", command=lambda: self.aplicaLetras(7))
        self.practica3.add_command(label="Naipes", command=lambda: self.aplicaLetras(8))
        self.menuBar.add_cascade(label="Práctica 3", menu=self.practica3)
    ...
    def aplicaLetras(self, opcion):
        if self.originalVentana.find_all() != ():
            ...
            self.entraY = IntVar()
            Entry(self.top, textvariable=self.entraY).pack()
            self.text = Label(self.top, text="Escribe el nombre del archivo html donde se va guardar ")
            self.text.pack()
            self.nombre = StringVar()
            Entry(self.top, textvariable=self.nombre).pack()
            self.buttontext = StringVar()
            self.buttontext.set("Aplica Letras")
            self.button = Button(self.top, textvariable=self.buttontext,
                                 command=lambda: self.obtenLetras(self.entraX, self.entraY, self.nombre, opcion)).pack()
        else:
            tkinter.messagebox.showwarning("Error", "Escoge una imagen antes de aplicar un filtro")


    def obtenLetras(self, valorX, valorY, nombreA, opcion):
        self.entraX = valorX.get()
        self.entraY = valorY.get()
        self.nombre = nombreA.get()
        if opcion == 1:
            letra(self.imagen, self.otra, self.entraX, self.entraY, self.nombre)
        elif opcion == 2:
            letraGris(self.imagen, self.otra, self.entraX, self.entraY, self.nombre)
        elif opcion == 3:
            letrasByN(self.imagen, self.otra, self.entraX, self.entraY, self.nombre)
        elif opcion == 4:
            letrasColor(self.imagen, self.otra, self.entraX, self.entraY, self.nombre)
        elif opcion == 5:
            letrasGrises(self.imagen, self.otra, self.entraX, self.entraY, self.nombre)
        elif opcion == 6:
            dominoB(self.imagen, self.otra, self.entraX, self.entraY, self.nombre)
        elif opcion == 7:
            dominoN(self.imagen, self.otra, self.entraX, self.entraY, self.nombre)
        elif opcion == 8:
            naipes(self.imagen, self.otra, self.entraX, self.entraY, self.nombre)
        self.top.destroy()


    def aplicaPalabras(self):
        if self.originalVentana.find_all() != ():
            self.top = Toplevel()
            self.label = Label(self.top, text="Introduce los valores del mosaico\ntienen que ser positivos para (x,y) ")
            self.label.pack()
            ...
            self.entraY = IntVar()
            Entry(self.top, textvariable=self.entraY).pack()
            self.peticion = Label(self.top, text="Escribe la oracion que llevara la imagen")
            self.peticion.pack()
            ...
            Entry(self.top, textvariable=self.nombre).pack()
            self.buttontext = StringVar()
            self.buttontext.set("Aplica Palabras")
            self.button = Button(self.top, textvariable=self.buttontext,
                                 command=lambda: self.obtenPalabras(self.entraX, self.entraY, self.nombre,
                                                                   self.cadena)).pack()
        else:
            tkinter.messagebox.showwarning("Error", "Escoge una imagen antes de aplicar un filtro")


    def obtenPalabras(self,valorX,valorY,nombreA,palabraA):
        self.entraX = valorX.get()
        self.entraY = valorY.get()
        self.nombre = nombreA.get()
        self.palabra = palabraA.get()
        palabras(self.imagen,self.otra,self.entraX,self.entraY,self.nombre,self.palabra)
        self.top.destroy()
